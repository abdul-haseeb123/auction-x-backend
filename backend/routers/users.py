...
async def read_root(request: Request, response: Response, db: Annotated[AsyncDatabase, Depends(get_db)]):
...
async def register_user(request: Request, db: Annotated[AsyncDatabase, Depends(get_db)]):
...
@router.post("/login")
async def login_for_access_token(form_data: Annotated[OAuth2PasswordRequestForm, Depends()], response:Response, db: Annotated[AsyncDatabase, Depends(get_db)]) -> ApiResponseToken:
    ...
    logged_user, access_token, refresh_token = await generate_access_refresh_token(TokenData(**user), db)
...
async def auth_with_google(request: Request, response: Response, db: Annotated[AsyncDatabase, Depends(get_db)]):
    try:
...
async def logout(current_user: Annotated[User, Depends(get_current_user)], response: Response, request: Request, db: Annotated[AsyncDatabase, Depends(get_db)]):
...
async def refresh_token(request: Request, response: Response, db: Annotated[AsyncDatabase, Depends(get_db)]):
    ...
    user = await users.get_google_user_by_refresh_token(refresh_token=refresh_token, db=db)
    if user:
        user_refresh_token = user["token"]["refresh_token"]
        token = await oauth.google.fetch_access_token(grant_type="refresh_token", refresh_token=user_refresh_token)
        print("here, ", token)
...
async def update_password(current_user: Annotated[User, Depends(get_current_user)], current_password:Annotated[str, Body()], new_password:Annotated[str, Body()], db: Annotated[AsyncDatabase, Depends(get_db)]):
    if current_user.account_type == "GOOGLE":
...
async def update_avatar(current_user: Annotated[User, Depends(get_current_user)], db: Annotated[AsyncDatabase, Depends(get_db)], avatar: UploadFile = File()):
    ...
    updated_user = await users.update_avatar(current_user.username, uploaded_avatar.model_dump(), db, new=True)
