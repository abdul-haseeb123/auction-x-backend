...
@pytest.mark.asyncio
async def test_login_user_with_invalid_credentials(test_client: AsyncClient, test_db: AsyncDatabase):
    response = await test_client.post("/users/login", data={
...
async def test_login_update_avatar(test_client: AsyncClient, test_db: AsyncDatabase):
    ...
    response = await test_client.put("/users/update-avatar", files={
    ...
    assert data["data"]["avatar"]["public_id"] != avatar_public_id, "Avatar public ID should be different after update"
    avatar_public_id = data["data"]["avatar"]["public_id"]
...
async def test_login_update_cover(test_client: AsyncClient, test_db: AsyncDatabase):
    ...
    response = await test_client.put("/users/update-cover-image", files={
        "cover_image": ("avatar.avif", open("assets/test/avatar.avif", "rb"), "image/avif")
    })
    assert response.status_code == 200
    data = response.json()
    assert data is not None
    ...
    assert data["data"]["cover_image"]["public_id"] != cover_image_public_id, "Cover image public ID should be different after update"
    cover_image_public_id = data["data"]["cover_image"]["public_id"]
    # Clean up the previous avatar image
