...
async def ensure_slug_index(db: AsyncDatabase):
    existing_indexes = [
        index["name"] async for index in await db.listings.list_indexes()
    ]
    if "slug_1" not in existing_indexes:
        try:
            await db.listings.create_index(
                [("slug", ASCENDING)], name="slug_1", unique=True
            )
...
async def create_listing(listing: dict, db: AsyncDatabase):
    try:
        ...
        owner = await users.get_user_by_username(owner_username, db)
        if not owner:
...
async def update_listing_bid(
    slug: str, data: dict, db: AsyncDatabase, new: bool = False
):
    return await db.listings.find_one_and_update(
        {"slug": slug},
        {"$push": {"bids": data}, "$set": {"current_bid": data["amount"]}},
        return_document=ReturnDocument.AFTER if new else ReturnDocument.BEFORE,
    )
...
async def delete_listing(username: str, listing_slug: str, db: AsyncDatabase):
    ...
    if str(listing["owner"]) != str(owner["username"]):
