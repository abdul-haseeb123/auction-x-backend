    ...
    if "username_1" and "email_1" not in existing_indexes:
        try:
            await db.users.create_index(
                [("username", ASCENDING)], name="username_1", unique=True
            )
...
async def update_refresh_token(
    username: str, refresh_token: str | None, db: AsyncDatabase, new=False
):
...
async def update_google_token(
    username: str, google_token: dict | None, db: AsyncDatabase, new=False
):
    return await db.users.find_one_and_update(
        {"username": username},
        {"$set": {"token": google_token}},
        return_document=ReturnDocument.AFTER if new else ReturnDocument.BEFORE,
    )
...
async def update_cover_image(
    username: str, cover_image: dict, db: AsyncDatabase, new=False
):
    return await db.users.find_one_and_update(
        {"username": username},
